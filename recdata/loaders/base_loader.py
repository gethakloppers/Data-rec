...
def _validate_config(config: dict[str, Any], config_path: Path) -> None:
    ...
    if "files" in config:
        ...
        if not isinstance(files, dict):
            ...
            # At least interactions must be defined
            ...
            for role, file_def in files.items():
                if file_def is None:
                    continue  # null means no file for this role (e.g., no user file)
                if not isinstance(file_def, dict):
                    errors.append(f"'files.{role}' must be a dict or null")
                    continue
                if "filename" not in file_def:
                    errors.append(f"'files.{role}' missing required key 'filename'")
                if "format" not in file_def:
                    errors.append(f"'files.{role}' missing required key 'format'")
                elif file_def["format"] not in VALID_FORMATS:
                    errors.append(
        ...
        if not isinstance(schema, dict):
            ...
            for key in REQUIRED_SCHEMA_KEYS:
                if key not in schema:
                    ...
                    errors.append(f"'schema.{key}' must be a non-empty list of candidate column names")
            ...
            if not isinstance(features, dict):
                ...
                for feat_type, columns in features.items():
                    ...
                    if not isinstance(columns, list):
                        errors.append(f"'{feature_key}.{feat_type}' must be a list of column names")

    # Validate stakeholder_roles (optional but must be well-formed if present)
